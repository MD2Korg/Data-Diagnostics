...
public class BatteryLevel {
	 ...
	 *            end time of a day
	 */
	public void phoneBatteryDown(List<DataPoints> rawBatteryLevels, List<DataPointQuality> blankWindows) {
		...
		for (int i = 0; i < blankWindows.size() - 1; i++) {
			...
			// difference
			/*
			 * if (((rawBatteryLevels.get(i).getTimestamp() -
			 * rawBatteryLevels.get(i + 1).getTimestamp()) >
			 * DDT_PARAMETERS.PHONE_SENSOR_DIFF) &&
			 * rawBatteryLevels.get(i).getValue() <=
			 * DDT_PARAMETERS.PHONE_BATTERY_DOWN) { phoneBatteryDown.add(new
			 * DataPoints(rawBatteryLevels.get(i).getTimestamp(),
			 * rawBatteryLevels.get(i+1).getTimestamp())); }
			 */
		}
	 ...
	 */
	/*
	 * public void phonePoweredOff(List<DataPoints> rawBatteryLevels) { //long
	 * phonePoweredOffTime = 0;
	 * 
	 * // iterate over battery levels to see if there is 0 battery level for
	 * (int i = 0; i < rawBatteryLevels.size() - 1; i++) {
	 * 
	 * // check if two stamps has more than 60 (threshold) seconds // difference
	 * if (((rawBatteryLevels.get(i).getTimestamp() - rawBatteryLevels.get(i +
	 * 1).getTimestamp()) > DDT_PARAMETERS.PHONE_SENSOR_DIFF) &&
	 * rawBatteryLevels.get(i).getValue() > DDT_PARAMETERS.PHONE_POWERED_OFF) {
	 * phonePoweredOffTime += (rawBatteryLevels.get(i).getTimestamp() -
	 * rawBatteryLevels.get(i + 1).getTimestamp()); phonePoweredOff.add(new
	 * DataPoints(rawBatteryLevels.get(i).getTimestamp(),rawBatteryLevels.get(i+
	 * 1).getTimestamp())); System.out.println("====="+phoneBatteryDown.size());
	 * }
	 * 
	 * }
	 * 
	 * //System.out.println("Phone Powered Off (MilliSeconds): " +
	 * phonePoweredOffTime);
	 * 
	 * }
	 */
	 ...
	 */
	/*
	 * public void sensorBatteryDown(List<DataPoints> rawBatteryLevels) { double
	 * voltageValue; //long sensorPoweredOffTime = 0; for (int i = 0; i <
	 * rawBatteryLevels.size()-1; i++) {
	 * 
	 * // ADC value to voltage voltageValue =
	 * (rawBatteryLevels.get(i).getValue() / 4096) * 3 * 2; if
	 * (((rawBatteryLevels.get(i).getTimestamp() - rawBatteryLevels.get(i +
	 * 1).getTimestamp()) > DDT_PARAMETERS.AUTOSENSE_SENSOR_DIFF) &&
	 * voltageValue <= DDT_PARAMETERS.AUTOSENSE_BATTERY_DOWN) {
	 * sensorPoweredOffTime += (rawBatteryLevels.get(i).getTimestamp() -
	 * rawBatteryLevels.get(i + 1).getTimestamp()); sensorBatteryDown.add(new
	 * DataPoints(rawBatteryLevels.get(i).getTimestamp(),rawBatteryLevels.get(i+
	 * 1).getTimestamp()));
	 * 
	...
	public void sensorBatteryDown(List<DataPoints> rawBatteryLevels, List<DataPointQuality> blankWindows) {
		long startTime, endTime;
		double voltageValue;
		// iterate over battery levels to see if there is 0 battery level
		for (int i = 0; i < blankWindows.size() - 1; i++) {
	 ...
	 */
	/*
	 * public void sensorPoweredOff(List<DataPoints> rawBatteryLevels) { double
	 * voltageValue; //long sensorOnTime = 0, sensorOffTime = 0; for (int i = 0;
	 * i < rawBatteryLevels.size()-1; i++) {
	 * 
	 * // ADC value to voltage voltageValue =
	 * (rawBatteryLevels.get(i).getValue() / 4096) * 3 * 2; if
	 * (((rawBatteryLevels.get(i).getTimestamp() - rawBatteryLevels.get(i +
	 * 1).getTimestamp()) > DDT_PARAMETERS.AUTOSENSE_SENSOR_DIFF) &&
	 * voltageValue >= DDT_PARAMETERS.AUTOSENSE_POWERED_OFF) { sensorOffTime +=
	 * (rawBatteryLevels.get(i).getTimestamp() - rawBatteryLevels.get(i +
	 * 1).getTimestamp()); sensorPoweredOff.add(new
	 * DataPoints(rawBatteryLevels.get(i).getTimestamp(),rawBatteryLevels.get(i+
	 * 1).getTimestamp())); }
	 * 
	 * } //sensorOnTime = (endDayTime - startDayTime) - sensorOffTime;
	 * 
	 * //System.out.println("Sensor ON : " + sensorOnTime + " - Sensor OFF : " +
	 * sensorOffTime); }
	 */
